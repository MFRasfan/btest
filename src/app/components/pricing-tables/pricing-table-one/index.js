...
import en from '@/app/data/i18n/subscription/en.json'
  ...
  return (
    <div className={`${styles.pricing} ${data.featured ? styles.active : ""}`}>
      <div className={styles.content} style={{ height: '100%' }}>
        ...
        <span style={{ textTransform: "uppercase" }}>{translate(String(data.planName).toLowerCase())}</span>
        ...
        <div className={styles.price}>
          <p>
            ...
            <Link href="/price-details">
              <p style={{ marginTop: -10, lineHeight: 0.47, color: "#888888" }}><sub>{translate(data.caption)}</sub></p>
            </Link>
          </p>
        </div>
        ...
        <div className={styles.price__btn}>
          <Button
            type="link"
            // url="/price-details"
            url={`price-comparison/price-comparison-${type}`}
            // text={t("compare")}
            text="Compare"
